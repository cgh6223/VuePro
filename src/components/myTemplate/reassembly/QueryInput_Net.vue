  ...
  export default {
    ...
    created() {
      if (!this.$myfunction.isEmpty(this.data)) {
        ...
        this.getLoadDataByUrl()
       // this.$emit("getData",this.loadData);
      }
    },
    //自定义函数节点
    methods: {
      ...
      getLoadDataByUrl() {
        if (!this.$myfunction.isEmpty(this.filterObj)) {
          GetHttp_Net('back_post', this.url, 'post', Object.assign({}, this.filterObj)).then(item => {
            this.loadData = item.data
          })
        } else if (!this.$myfunction.isEmpty(this.url)) {
          GetHttp_Net('back_post', this.url, 'post').then(item => {
            console.log(item);
            this.loadData = item.data
          })
        }
