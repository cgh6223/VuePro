<template>
  <div class="text-center">
   ...
   />-->
          ...
          <el-dialog title="组织架构" width="30%" :close-on-click-modal="false" :close-on-press-escape="false"
                     :modal-append-to-body="false"
                     :visible.sync="orgDialogShow">
            <el-form ref="searchform" :model="addform" label-width="120px" style="margin:10px;width:auto;">
      ...
      <el-col :span="16">
        <el-card>
          <span>岗位</span>
          <position-manager-index></position-manager-index>
        </el-card>
      </el-col>
    </el-row>
...
<script>
  ...
  import {getOrgTree, addOrgTree, modifyOrgTree} from "../../../api/old/system";
  import PositionManagerIndex from '../PositionManager/index.vue'

  export default {
    components: {
      PositionManagerIndex
    },
    data() {
      return {
        data: [
            ...
            children: [
              {
                ...
                pid: null,
                children: [
                  {label: "经营部", id: "7", pid: null, children: []},
                  {label: "行政部", id: "8", pid: null, children: []},
                  {label: "分支机构", id: "9", pid: null, children: []},
                  {label: "自营分公司", id: "10", pid: null, children: []}
                ]
              },
              {
                label: "造价中心",
                ...
                children: [
                  {
                    ...
                    pid: null,
                    children: [
                      {label: "一部", id: "15", pid: null, children: []},
                      {label: "二部", id: "16", pid: null, children: []},
                      {label: "三部", id: "17", pid: null, children: []}
                    ]
                  },
                  {label: "造价贵阳部", id: "12", pid: null, children: []}
                ]
              },
              {
                label: "监理中心",
                ...
                pid: null,
                children: [
                  {label: "监理贵阳部", id: "13", pid: null, children: []}
                ]
              },
              {
                label: "咨询中心",
                ...
                pid: null,
                children: [
                  {label: "咨询贵阳部", id: "14", pid: null, children: []}
                ]
              },
              {label: "财务部", id: "6", pid: null, children: []}
            ]
          }
        ],
        horizontal: false,
    ...
    methods: {
      getList() {
        getOrgTree().then(res => (this.data = res));
      },
      append(d) {
        this.addform.type = "add";
        this.addform.mydata = d;
        this.orgDialogShow = true;
      },
      modify(n, d) {
        this.addform.type = "modify";
        this.addform.orgName = d.label;
        this.addform.mydata = d;
        this.orgDialogShow = true;
      },
      addOrg(myd) {
        ...
        if (this.addform.orgName === "") {
          ...
          if (this.addform.type === "add") {
            ...
            let o = {
              ...
              parentId: this.addform.mydata.id
            };
            ...
            // }
            o = {row: JSON.stringify(o)};
            addOrgTree(o).then(res => {
              if (res.sucess) {
                this.$message({
                  message: "数据更新成功！！！",
                  type: "success"
            ...
            let o = {
              keyid: this.addform.mydata.id,
              orgName: this.addform.orgName,
              parentId: this.addform.mydata.pid
            };
            o = {row: JSON.stringify(o)};
            modifyOrgTree(o).then(res => {
              if (res.sucess) {
                this.$message({
                  message: "数据更新成功！！！",
                  type: "success"
