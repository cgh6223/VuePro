...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      'form.uncertainServiceDay': function (newValue, oldValue) {
        if (newValue === '') {
          newValue = '0'
        }
        this.form.uncertainDay = parseInt(newValue)
        let oldDay = 0
        if (oldValue !== '') {
          oldDay = oldValue
        }
        if (this.form.planServiceDays === '') {
          this.form.planServiceDays = 0 + this.form.uncertainDay - oldDay
        } else {

          this.form.planServiceDays = this.form.planServiceDays + this.form.uncertainDay - oldDay
        }
      },
    ...
    //生命周期函数节点
    created() {
      this.networkGetSelectByCode(this.currentItem.serviceCode).then(item=>{
