...
<script>
  ...
  export default {
    ...
    computed:{
        ...mapState('projectConsultWorkGroup',{
          projectConsultWorkGroup:'projectConsultWorkGroup'
        }),
      ...mapState('loadType',{
    ...
    //生命周期函数节点
    created() {
      this.networkGetSelect(['projectConsultWorkGroup'])
      this.networkGetSelectByCode('TYPE-064').then(item=>{
        this.workGroup=item
      })
    },
    //自定义函数节点
